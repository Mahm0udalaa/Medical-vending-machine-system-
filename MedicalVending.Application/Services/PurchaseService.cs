...
namespace MedicalVending.Application.Services
{
    public class PurchaseService : IPurchaseService
    {
        ...
        public async Task<PurchaseDto> CheckoutCartAsync(int customerId)
        {
            ...
            // 4. Validate medicines and stock
            ...
            // Check stock in MachineMedicine
            foreach (var item in cartItems)
            {
                // You may need to inject IMachineMedicineRepository for this in a real clean architecture
                // For now, let's assume you have a way to check and update stock here
                // (You may want to refactor this part for full clean architecture)
                // var machineMedicine = await _machineMedicineRepository.GetByIdAsync(item.MachineId, item.MedicineId);
                // if (machineMedicine == null || machineMedicine.Quantity < item.Quantity)
                //     throw new BadRequestException($"Not enough stock for medicine ID {item.MedicineId} in machine.");
            }
            ...
            foreach (var item in cartItems)
            {
                ...
                // Update stock in MachineMedicine (see note above)
            }
